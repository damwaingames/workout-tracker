...
- `slot.js` — the slot ctx contract: builds a **Performance**'s six-part ctx (and an **Attendance**'s coarser one) into `data-*` and reads it back off a dataset, plus the in-slot field-name vocabulary. Pure; imported by `render`/`compose`/`events` so the render→handler agreement lives in one module.
