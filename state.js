...
import {
  ...
  placement, loadMode, circuitOf, clampSets, clampRounds, uniqueId, validYMD, bandFor, bandKg, scheduledDate, fmtYMD,
} from "./helpers.js";
...
// A single logged day as a Health Connect NutritionRecord payload, or null when that day
// logged no food. `clientId` is the cell key — a *stable* per-day id, so re-syncing a day
// UPSERTS its record instead of duplicating it (Health Connect keys upserts on
// clientRecordId; the native companion app supplies clientRecordVersion = write time).
// `date` is the routine's calendar day (block start + week offset + its position in that
// week's schedule — ADR-0005, so a reorder moves the date, never the cell key). The four
// nutrients are the day's derived totals rounded to the same whole figures the card shows
// (fmt), and are built from NUTRIENTS so the nutrient set keeps its one home. This is the
// pure-PWA half of the Health Connect export — the write itself is native (health-connect-backlog).
export function nutritionRecord(block, wk, routine) {
  ...
  const nutrients = Object.fromEntries(NUTRIENTS.map((x) => [x.id, Math.round(n[x.id])]));
  return { clientId: cell, date: fmtYMD(scheduledDate(block.startDate, wk, position)), ...nutrients };
}

// Every logged day across every block as NutritionRecord payloads — the block × week ×
// routine walk with empty days dropped, for a full Health Connect backfill/export. The
// per-day unit (nutritionRecord) is what a future write-on-log path re-emits for a single
// edited day; this is the initial scan over that same builder.
export function nutritionRecords() {
  const out = [];
