...
/* Collapsible days: the chevron toggle, the persisted `.collapsed` flag (absent =
 * expanded), the animated body slide (grid 1fr↔0fr → height to ~0), the collapsed-
 * only totals summary per day kind (strength volume / recovery circuit-time),
 * auto-collapse on done + auto-expand on un-done, and survival across a reload. */
verify(async ({ page, ck, ls, reset }) => {
  const D1 = '.day[data-cell="b1.w1.d1"]'; // strength — Workout A
  const D2 = '.day[data-cell="b1.w1.d2"]'; // recovery — Recovery A (no banded moves)
  const isCollapsed = (sel) => page.$eval(sel, (el) => el.classList.contains("is-collapsed"));
  const ariaExpanded = (sel) => page.getAttribute(`${sel} .day-collapse`, "aria-expanded");
  const flag = async (cell) => (await ls()).log[`${cell}.collapsed`];
  const bodyHeight = async (sel) => { const b = await page.locator(`${sel} .day-collapsible`).boundingBox(); return b ? b.height : 0; };
  ...
  ck("day starts expanded", !(await isCollapsed(D1)));
  ck("expanded: aria-expanded is true", (await ariaExpanded(D1)) === "true");
  ck("expanded: summary hidden", !(await page.isVisible(`${D1} .day-summary`)));
  ...
  ck("collapsed: summary visible", await page.isVisible(`${D1} .day-summary`));
  ck("collapsed flag persisted true", (await flag("b1.w1.d1")) === true);
  ck("strength summary shows tonnage (Volume … kg)", /Volume.*kg/.test(await page.textContent(`${D1} .day-summary`)));
  ...
  await page.waitForTimeout(60);
  const s2 = await page.textContent(`${D2} .day-summary`);
  ...
  await page.waitForTimeout(60);
  ck("marking a day done auto-collapses it", await isCollapsed(D1));
