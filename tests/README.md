...
| `verify-compose` | Authoring a block (#45): a new block is blank (not a clone), a day switches Session/Class/Rest, a Session grows Groups + Items with editable rails/rounds/rests (→ superset), weeks + start date are editable, days reorder, and the composed plan renders + persists. Also (#64) switching a *logged* day's kind confirms first, quoting what it would retire — cancelling leaves plan + history alone, accepting retires the set rather than losing it |
| `verify-plan` | **Pure-Node** (no browser): the plan-editing verbs (#64) — removing a Group / Item repairs the surviving slots' Performance back-references (the reported mis-prefill: a surviving slot showing a *removed* Group's numbers) and retires the removed one's, moving a Group swaps them, a kind switch retires the replaced Routine's sets + Attendances + Session RPE while a Holiday-swapped week keeps its own, the same verbs edit the Holiday Session over the swapped cells, the day-reorder (moved here from `verify-tonnage`), the block-length clamp, the scalar clamps + allow-lists, and the orphan count the confirm quotes |
...
| `verify-tonnage` | **Pure-Node** (no browser): the #47 maths + queries — a rep set carries an optional RIR marker (not station/steady), the advisory RIR nudge, and the loading-mode-honest tonnage readout (session + block, per-side reps / two-dumbbell weight, bodyweight adds nothing, a rest day / unlogged week zero). The `swapDays` assertions moved to `verify-plan` with the verb (#64) |
...
| `verify-equipment` | **Pure-Node** (no browser): the #48 equipment + Holiday-swap queries — `fitsKit` (equipment ⊆ kit), `awayEligible` (⊆ the away kit, retired excluded), the per-cell `.holiday` flag + `effectiveRoutine` (Holiday Session vs planned), a holiday day leaving the planned exercise's ghost untouched, and `swapDays` carrying the `.holiday` flag with a reordered day |
| `verify-holiday` | Equipment + Holiday Session surfaced (#48, in-browser): an exercise's equipment shows in the Library, the Holiday editor's picker offers only away-eligible exercises (band/bodyweight present, dumbbell absent) and grows the Session, and the 🏝 toggle swaps the Holiday Session into a day — its band work logs while the planned move gets none, persisted across reload and reversible |
| `verify-winddown` | Wind-down daily habit (#49, in-browser): a seven-night strip for the current calendar week (frozen clock) with a done/target adherence readout, a future night not yet tickable, a legacy per-cell `.winddown` tick re-homed onto the date-keyed habit, toggling a night persisting across reload, and the Edit-mode target live-updating the readout |
| `verify-class-library` | Class logging + Library edit/retire (#50, in-browser): a Class routine logs actual minutes / wearable kcal / note as an Attendance (re-hydrated + shown in the Library), an exercise's fields edit in the Library with a loading-mode correction doubling session tonnage over history, and an exercise retires (gone from the composer picker, history intact) + un-retires |
