...
| `verify-progression` | **Pure-Node** (no browser): the #46 maths + queries — rail→zone threading (floor label + spanned set), the double-progression target across kg / band / bodyweight and at the rail cap, the adjustable-dumbbell discrete-load ladder (ADR-0031 — kg steps snap to a real rung, a maxed load climbs reps), Epley guide-ghost seeding + snap, the e1RM trend, and the ghost / target / guide / load-carries-across-a-rail-change behaviour over a real timeline |
